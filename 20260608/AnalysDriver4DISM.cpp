...
int main()
{
   ...
   do  {
      if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
         ...
         fprintf(f, "Dism /Add-Driver /Image:\"c:\\WinPE_x64\\mount\" /Driver:\"%s\\%s\"\n", InfoDir, ffd.cFileName);
         wsprintf(tmp, "Dism /Add-Driver /Image:\"c:\\WinPE_x64\\mount\" /Driver:\"%s\\%s\"", InfoDir, ffd.cAlternateFileName);
         system(tmp);
      }
   }
...
void Add2DISM(LPTSTR lpszFunction) {
    char tmp[260];
    wsprintf(tmp, "Dism /Add-Driver /Image:\"c:\\WinPE_x64\\mount\" /Driver:\"%s\\%s\"",InfoDir, lpszFunction);
    system(tmp);
